let designcl = ['Work','Personal','School', 'Cleaning','Others'] //creating class for implementing design to different category 
...
function checkedOrNot(){ 
    ...
    let ddsp = document.querySelectorAll('.dueD'); // getting all the class for dueD
    for(let i=0;i<descdisp.length;i++){
        let dueD = ddsp[i].innerHTML;
        // checking if checkbox is checked  if checked a line will pass through the text(-) else if it is unchecked no line will pass through date and description
            ...
            document.getElementById(cb[i].getAttribute('uid')+dueD).style.textDecoration  = 'line-through'
            }
            else if(cb[i].checked == false){
            document.getElementById(cb[i].getAttribute('uid')).style.textDecoration = 'none'
            document.getElementById(cb[i].getAttribute('uid')+dueD).style.textDecoration  = 'none'
        }
...
document.getElementById('deleteButton').addEventListener('click',function(){
    ...
    if(arrcheck.length===0){ // checking if array is null the 
        ...
        swal("No Item Checked!!", "Please Select An Item To Remove!", "error"); // using show alert to show if there is no items in the array
        return;
    }
    ...
    $.ajax({
        ...
        success: function(){ // on ajax sunnces i.e when data is delete
            swal("All The Selections Deleted ", "Click OK To Return", "success") // using sweet alert to show the data is delete
            .then(redir => {
                window.location = '/';
            })
